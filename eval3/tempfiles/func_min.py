...
class Genetic(object):
    ...
    def initializePopulation(self):
        return [np.random.uniform(self.minim, self.maxim, size=(self.n_variables)) 
                           for i in range(self.pop_size)]
    ...
    def nextGen(self):
        ...
        while len(children) < self.pop_size:
            ...
            # Crossover
            alpha = random.random()
            child = [alpha * p1[i] + (1 - alpha) * p2[i] for i in range(self.n_variables)]
